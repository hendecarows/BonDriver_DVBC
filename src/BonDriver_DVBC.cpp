...
// The original code from https://github.com/u-n-k-n-o-w-n/BonDriverProxy_Linux

#include "BonDriver_DVBC.h"
...
static BOOL g_TsmfDeleteNull;	// 0 : Nullパケット保持 / 1 : 削除 (TSMF分離時のみ)
...
static int Init()
{
	...
	while (::fgets(buf, sizeof(buf), fp))
	{
		...
		if ((idx != 0) && IsTagMatch(buf, "#ISDB_S", NULL))
			...
			idx = 1;
		else if ((idx != 2) && IsTagMatch(buf, "#DVB_C", NULL))
			idx = 2;
		...
		else if (!bfFlag && IsTagMatch(buf, "#FRONTEND_NO", &p))
		{
			g_FrontendNo = ::atoi(p);
			bfFlag = TRUE;
		}
		else if (!bxFlag && IsTagMatch(buf, "#DEMUX_NO", &p))
		{
			g_DemuxNo = ::atoi(p);
			bxFlag = TRUE;
		}
		else if (!bvFlag && IsTagMatch(buf, "#DVR_NO", &p))
		{
			g_DvrNo = ::atoi(p);
			bvFlag = TRUE;
		}
		else if (!beFlag && IsTagMatch(buf, "#DELIVERYSYSTEM", &p))
		{
			g_Type = ::atoi(p);
			beFlag = TRUE;
		}
		...
		else if (!bnFlag && IsTagMatch(buf, "#TSMFDELETENULL", &p))
		{
			g_TsmfDeleteNull = ::atoi(p);
			bnFlag = TRUE;
		}
...
cBonDriverDVB::cBonDriverDVB() : m_fifoTS(m_c, m_m), m_fifoRawTS(m_c, m_m), m_StopTsSplit(m_c, m_m), m_StopTsmfParser(m_c, m_m)
{
	m_spThis = this;
	...
	m_hTsRead = m_hTsSplit = m_hTsmfParser = 0;
...
cBonDriverDVB::~cBonDriverDVB()
{
	...
	{
		LOCK(m_writeLock);
		TsFlush((g_Type == 2) || g_UseServiceID);
		delete m_LastBuf;
	}
...
const BOOL cBonDriverDVB::OpenTuner(void)
{
...

	{
		...
		m_fefd = ::open(os.str().c_str(), O_RDWR | O_CLOEXEC);
	}
	if (m_fefd < 0)
	{
	...
	else if (g_Type == 1)	// ISDB_T
	{
		prop[0].u.data = SYS_ISDBT;
		p = (char *)"UHF";
	}
	else if (g_Type == 2)	// DVB_C
	{
		prop[0].u.data = SYS_DVBC_ANNEX_A;
		p = (char *)"CATV";
	}
	else
	{
		::fprintf(stderr, "OpenTuner() adapter type unknown = %d: adapter%d \n", g_Type, g_AdapterNo);
		goto err1;
	}
	...
	{
		std::ostringstream os;
		os << "/dev/dvb/adapter" << g_AdapterNo << "/demux" << g_DemuxNo;
		m_dmxfd = ::open(os.str().c_str(), O_RDONLY | O_CLOEXEC);
	}
	if (m_dmxfd < 0)
	{
	...
	{
		std::ostringstream os;
		os << "/dev/dvb/adapter" << g_AdapterNo << "/dvr" << g_DvrNo;
		// 休止チャンネルなどでCloseTuner()がブロックしてしまうのでO_NONBLOCKが必要
		m_dvrfd = ::open(os.str().c_str(), O_RDONLY | O_NONBLOCK | O_CLOEXEC);
	}
	if (m_dvrfd < 0)
	{
...
const BOOL cBonDriverDVB::SetChannel(const DWORD dwSpace, const DWORD dwChannel)
{
	...
	if (bFlag)
	{
		...
		{
			f = (GetFrequency_T(g_stChannels[g_Type][dwChannel].freq.frequencyno) / 1000000) * 1000000;
			prop[0].cmd = DTV_FREQUENCY;
			prop[0].u.data = f;
			prop[1].cmd = DTV_MODULATION;
			prop[1].u.data = QAM_AUTO;
			prop[2].cmd = DTV_INVERSION;
			prop[2].u.data = INVERSION_AUTO;
			prop[3].cmd = DTV_SYMBOL_RATE;
			prop[3].u.data = 5274000;
			prop[4].cmd = DTV_TUNE;
			props.props = prop;
			props.num = 5;
		}
		...
		{
			::fprintf(stderr, "SetChannel() adapter type unknown: adapter%d\n", g_AdapterNo);
			goto err;
		}

		BOOL bOk = FALSE;
		for (int i = 0; i < 3; i++)	// 250ms * 4 * 3で最大3秒
		{
			if (::ioctl(m_fefd, FE_SET_PROPERTY, &props) < 0)
			{
...

	{
		...
		if (g_Type == 2)
		{
			auto tsid = g_stChannels[g_Type][dwChannel].freq.tsid;
			if (tsid < 1 || tsid > 15)
			{
				m_TsmfParser.SetTSID(0, 0xffff, TRUE);
				m_bParseTsmf = FALSE;
			}
			else
			{
				m_TsmfParser.SetTSID(0, tsid - 1, TRUE);
				m_bParseTsmf = TRUE;
			}
		}
	}
	...
	if (!m_hTsRead)
	{
		...
		{
			::perror("pthread_create TsReader");
			goto err;
		}
	}
...
void *cBonDriverDVB::TsReader(LPVOID pv)
{
	...
	if (g_Type == 2)
	{
		if (::pthread_create(&(pDVB->m_hTsmfParser), NULL, cBonDriverDVB::TsmfParser, pDVB))
		{
			::perror("pthread_create TsmfParser");
			ret = 302;
			return &ret;
		}
	}
	else if (g_UseServiceID)
	{
		if (::pthread_create(&(pDVB->m_hTsSplit), NULL, cBonDriverDVB::TsSplitter, pDVB))
		{
			::perror("pthread_create TsSplitter");
			ret = 301;
			return &ret;
		}
	}
	...
	if (g_Type == 2)
	{
		pDVB->m_StopTsmfParser.Set();
		::pthread_join(pDVB->m_hTsmfParser, NULL);
		pDVB->m_hTsmfParser = 0;
		pDVB->m_StopTsmfParser.Reset();
	}
	else if (g_UseServiceID)
	{
...
void *cBonDriverDVB::TsmfParser(LPVOID pv)
{
	auto pBon = static_cast<cBonDriverDVB *>(pv);
	std::unique_ptr<BYTE[]> pTsBuf(new BYTE[TS_BUFSIZE]);
	std::unique_ptr<TS_DATA> pRawBuf;
	std::unique_ptr<BYTE[]> pNewBuf;
	size_t tsBufPos = 0;
	cEvent *h[] = { &(pBon->m_StopTsmfParser), pBon->m_fifoRawTS.GetEventHandle() };

	while (true)
	{
		auto ret = cEvent::MultipleWait(2, h);
		switch (ret)
		{
		case WAIT_OBJECT_0:
			goto end;
		...
		case WAIT_OBJECT_0 + 1:
		{
			TS_DATA* pRawBuf0 = nullptr;
			pBon->m_fifoRawTS.Pop(&pRawBuf0);
			if (pRawBuf0 == nullptr)
				break;

			if (!pBon->m_bParseTsmf)
			{
				// TSMF分離なしの場合はそのままTSデータを移動
				pBon->m_fifoTS.Push(pRawBuf0);
				break;
			}

			BYTE* pNewBuf0 = nullptr;
			size_t newBufLen = 0;
			pBon->m_TsmfParser.ParseTsBuffer(pRawBuf0->pbBuf, pRawBuf0->dwSize, &pNewBuf0, &newBufLen, g_TsmfDeleteNull);
			pRawBuf.reset(pRawBuf0);
			pNewBuf.reset(pNewBuf0);
			if (newBufLen == 0 || !pNewBuf0)
				break;

			size_t newBufPos = 0;
			while (newBufPos < newBufLen)
			{
				auto restLen = newBufLen - newBufPos;
				auto copyLen = restLen < (TS_BUFSIZE - tsBufPos) ? restLen : (TS_BUFSIZE - tsBufPos);
				::memcpy(pTsBuf.get() + tsBufPos, pNewBuf.get() + newBufPos, copyLen);

				tsBufPos += copyLen;
				newBufPos += copyLen;
				if (tsBufPos == TS_BUFSIZE)
				{
					TS_DATA *pData = new TS_DATA();
					pData->dwSize = TS_BUFSIZE;
					pData->pbBuf = pTsBuf.release();
					pBon->m_fifoTS.Push(pData);
					pTsBuf.reset(new BYTE[TS_BUFSIZE]);
					tsBufPos = 0;
				}
			}
		}
